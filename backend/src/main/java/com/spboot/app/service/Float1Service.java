...
@Service
public class Float1Service {

    // 获取数据库操作类mapper
    @Resource
    private Float1Mapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Float1> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Float1>> selectAll(Wrapper<Float1> query) {
    ...
    public R<List<Float1>> selectAll() {
        QueryWrapper<Float1> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Float1> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Float1> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Float1> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Float1> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Float1 entityData, Map post) {
        // 判断是否有填写标题。
        if (StringUtil.isNullOrEmpty(post.get("title"))) {
            return R.error("请填写标题");
        }

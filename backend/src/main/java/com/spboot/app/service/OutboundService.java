...
public class OutboundService {
    ...
    public QueryWrapper<Outbound> mapToWrapper(Map<String, Object> map) {
        ...
        // 判断URL 参数goodsid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("goodsid")) && Convert.toInt(map.get("goodsid")) > 0) {
            // 大于0 则写入条件
    ...
    public R insert(Outbound entityData, Map post) {
        // 判断是否有填写出库编号。
        if (StringUtil.isNullOrEmpty(post.get("numbers1"))) {
            return R.error("请填写出库编号");
        }
        // 判断是否有填写出库数量。
        if (StringUtil.isNullOrEmpty(post.get("quantity"))) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("UPDATE outbound SET subtotal = (price*quantity) WHERE id='" + charuid + "'");

            DB.execute("UPDATE goods SET inventory = inventory-'" + post.get("quantity") + "' WHERE id='" + post.get("goodsid") + "'");
    ...
    public R<Object> update(Outbound entityData, Map post) {
        ...
        Integer charuid = entityData.getId();
        DB.execute("UPDATE outbound SET subtotal = (price*quantity) WHERE id='" + post.get("id") + "'");

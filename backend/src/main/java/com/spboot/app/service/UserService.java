...
public class UserService {
    ...
    public R<Object> login(String username, String pwd, String cx) {
        ...
        if (cx.equals("用户")) {
            // 通过User1Service.login 方法进行登录

            User1 user = user1Service.login(username, pwd);
            // user 为null 则提示用户账号或密码错误
            if (user == null) {
                return R.error("账号或密码错误");
            }
            ...
            // 设置table 变量
            // 将bean 实体类转化为session
            session = user1Service.toSession(user);
        }
        // 判断是否为"员工"角色登录时则使用这个进行登录
        if (cx.equals("员工")) {
            // 通过StaffService.login 方法进行登录

            Staff user = staffService.login(username, pwd);
            // user 为null 则提示用户账号或密码错误
            if (user == null) {
                return R.error("账号或密码错误");
            }
            ...
            // 设置table 变量
            // 将bean 实体类转化为session
            session = staffService.toSession(user);
        }
    ...
    public R<Object> editPassword(PasswordPojo passwordPojo) {
        ...
        if (roles.equals("用户")) {
            return user1Service.editPassword(SessionFactory.getId(), passwordPojo.getOldPassword(), passwordPojo.getNewPassword());
        }

        if (roles.equals("员工")) {
            return staffService.editPassword(SessionFactory.getId(), passwordPojo.getOldPassword(), passwordPojo.getNewPassword());
        }

...
@Service
public class ProgressmerchbillService {

    // 获取数据库操作类mapper
    @Resource
    private ProgressmerchbillMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Progressmerchbill> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Progressmerchbill>> selectAll(Wrapper<Progressmerchbill> query) {
    ...
    public R<List<Progressmerchbill>> selectAll() {
        QueryWrapper<Progressmerchbill> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Progressmerchbill> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Progressmerchbill> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Progressmerchbill> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("progressid"))) {
    ...
    public R selectPages(QueryWrapper<Progressmerchbill> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Progressmerchbill entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("UPDATE progressmerchbill SET subtotal = (shangpinjiage*goumaishuliang) WHERE id='" + charuid + "'");
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Progressmerchbill entityData, Map post) {
        mapper.updateById(entityData);

        Integer charuid = entityData.getId();
        DB.execute("UPDATE progressmerchbill SET subtotal = (shangpinjiage*goumaishuliang) WHERE id='" + post.get("id") + "'");

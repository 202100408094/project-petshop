...
import com.spboot.app.service.User1Service;
...
public class User1Controller {
    ...
    public R<List<User1>> selectAll() {
        return user1Service.selectAll();
    }

    @ApiOperation(value = "根据条件筛选获取管理员列表，并分页", httpMethod = "POST")
    @RequestMapping("/selectPages")
    public R selectPages(@RequestBody Map<String, Object> req) {
        return user1Service.selectPages(req);
    }
    ...
    @ApiImplicitParam(name = "data", value = "使用json数据提交", type = "json", dataTypeClass = User1.class, paramType = "body")
    public R update(@RequestBody Map data) {
        User1 post = BeanUtil.mapToBean(data, User1.class, true);
        return user1Service.update(post, data);
    }
    ...
    public R insert(@RequestBody Map data) {
        User1 post = BeanUtil.mapToBean(data, User1.class, true);
        return user1Service.insert(post, data);
    }

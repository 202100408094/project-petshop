            ...
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column label="是否支付" width="120">
                        <template #default="{row}">
                            ...
                            <el-button
                                size="small"
                                type="primary"
                                @click="$goto({path:'/admin/pay' , query:{biao:'progress' , id:row.id,ordersn:row.orders,zongji:row.money,isShaxiang:'' } })"
                                v-if="row.iszf=='否'"
                            >
                                去支付
                            </el-button>
                    ...
                    <el-table-column label="操作" fixed="right" width="220">
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="row.state == '已发货'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/signfor/add?id='+row.id)">签收</el-button>
                                </template>
                                <template v-if="row.state == '待发货'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/cancel/add?id='+row.id)">取消</el-button>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/progress/selectUserss", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

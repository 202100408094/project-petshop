<template>
    <div class="views-outbound-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 用品出库查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="numbers1" label="出库编号" width="130">
                        <template #default="{row}"> {{ row.numbers1 }} </template>
                    </el-table-column>
                    <el-table-column prop="quantity" label="出库数量" width="120">
                        <template #default="{row}"> {{ row.quantity }} </template>
                    </el-table-column>
                    <el-table-column prop="subtotal" label="出库小计" width="120">
                        <template #default="{row}"> {{ row.subtotal }} </template>
                    </el-table-column>
                    <el-table-column prop="describes" label="出库说明"width="130">
                        <template #default="{row}"> {{ row.describes }} </template>
                    </el-table-column>
                    <el-table-column prop="handler" label="经手人" width="180">
                        <template #default="{row}"> {{ row.handler }} </template>
                    </el-table-column>
                    <el-table-column prop="addtime" label="出库时间"width="130">
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/outbound/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <template v-if="$session.cx == '管理员'">
                                    <el-tooltip effect="dark" content="编辑" placement="top-start"
                                        ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/outbound/updt?id='+row.id)">编辑</el-button>
                                    </el-tooltip>
                                </template>
...
<script setup>
    ...
    import { canOutboundSelect, useOutboundSelect, canOutboundDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canOutboundDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/outbound/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

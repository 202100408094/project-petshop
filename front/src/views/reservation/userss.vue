        ...
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="notes" label="预约备注"width="180">
                    ...
                    <el-table-column label="是否支付" width="120">
                        <template #default="{row}">
                            ...
                            <el-button
                                size="small"
                                type="primary"
                                @click="$goto({path:'/admin/pay' , query:{biao:'reservation' , id:row.id,ordersn:row.numberss,zongji:row.price,isShaxiang:'' } })"
                                v-if="row.iszf=='否'"
                            >
                                去支付
                            </el-button>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                ...
                                <el-tooltip effect="dark" content="删除" placement="top-start"
                                    ><el-button v-if="row.state == '待付款'" type="danger" :icon="Delete" size="small" @click="deleteItems(row.id)">删除</el-button>
                                </el-tooltip>
                            </el-button-group>
                        </template>
                    </el-table-column>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/reservation/selectUserss", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

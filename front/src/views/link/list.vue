        ...
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="names" label="网站名称">
                        <template #default="{row}"> {{ row.names }} </template>
                    </el-table-column>
                    <el-table-column prop="url" label="网址" width="130">
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/link/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canLinkSelect, useLinkSelect, canLinkDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/link/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

<template>
    <div class="views-arrange-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 服务安排查询 </span>
                    </div>
                </template>
                    ...
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="安排时间">
                            <div style="display: flex">
                                <el-date-picker v-model="search.times_start" style="width: 120px" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" placeholder="选择开始日期" />
                                -
                                <el-date-picker v-model="search.times_end" style="width: 120px" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" placeholder="选择结束日期" />
                            </div>
                        </el-form-item>
                        <el-form-item label="工号">
                            <el-input v-model="search.number1"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="times" label="安排时间" width="120">
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/arrange/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/arrange/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canArrangeSelect, useArrangeSelect, canArrangeDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/arrange/selectUserss", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
